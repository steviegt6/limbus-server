...
public class ExtendedAsmResolverDummyDllOutputFormat : Cpp2IlOutputFormat {
    private class StubAssemblyResolver : IAssemblyResolver {
        ...
        public AssemblyDefinition? Resolve(AssemblyDescriptor assembly) {
            return DummyAssemblies.TryGetValue(assembly.Name!, out var ret) ? ret : null;
        }
    ...
    public override string OutputFormatId => "extended-dummydll";

    public override string OutputFormatName => "Extended stub (dummy) DLL files.";
    ...
    private List<AssemblyDefinition> BuildStubAssemblies(ApplicationAnalysisContext context) {
        var assemblyResolver = new StubAssemblyResolver();
